...
//Creamos clase PDF que herada de FPDF
	class PDF extends FPDF
	{
		...
		function Header()
		{
            ...
            $this->Cell (0,5,utf8_decode('LISTADO DE MÓDULOS'),'0','0','C');
            $this->Ln(12);
	...
	// Movernos a la derecha
	$pdf->Cell(40,5,utf8_decode('PROGRAMA: '),1,0,'C');	
	$pdf->SetFont('Times','B',12);
	$pdf->Cell(150,5,utf8_decode(nombrePrograma($programa)),1,0,'C');			
	$pdf->Ln();
	$pdf->SetFont('Times','',12);	
	$pdf->Cell(40,5,utf8_decode('SEMESTRE: '),1,0,'C');		
	$pdf->SetFont('Times','B',12);
	$pdf->Cell(150,5,utf8_decode($semestre),1,0,'C');			
	$pdf->Ln(10);
	// Movernos a la derecha
	$pdf->Cell(30);
	$pdf->SetFont('Times','B',11);    	
	$pdf->Cell(13, 8, '#', 1, 0 ,'C', 0);
	$pdf->Cell(120, 8, utf8_decode('MÓDULO'), 1, 1 ,'C', 0);
    $sql = "SELECT * FROM modulo WHERE programa_id = $programa AND semestre= $semestre";
                    ...
                    
    {
            ...
            $pdf->Cell(120,5,utf8_decode($nom_modulo),1,1,'C');
